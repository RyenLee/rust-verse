...
// Auto-refresh when toolchain data changes from other pages
const stopToolchainWatch = onToolchainChange(() => {
  loadData()
})

// Auto-refresh when env var state changes (apply/deactivate/edit)
...
onActivated(() => {
  // Keep-alive: do NOT reload on re-activation, user must click refresh
})
</script>
...
<template>
  <div class="h-full overflow-y-auto">
    ...
    <!-- Dashboard content (always show when loaded, with warning banner if rustup missing) -->
    <div v-else class="p-8 lg:p-10 max-w-6xl mx-auto space-y-10">
      <!-- Rustup not installed warning banner -->
      <div
        v-if="envCheck && !envCheck.rustup_installed"
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1 space-y-2">
            ...
            <p
              ...
              v-html="t('dashboard.rustNotFoundDesc', { rustup: '<strong>rustup</strong>' })"
            />
            <div
              v-if="envCheck.rustup_error"
              class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 text-left"
            >
            ...
            <div class="flex gap-3 pt-1">
              <a
                ...
                target="_blank"
                class="inline-flex items-center gap-2 bg-orange-600 hover:bg-orange-500 text-white text-sm font-semibold py-2 px-4 rounded-lg transition-colors"
              >
              ...
              <a
                ...
                target="_blank"
                class="inline-flex items-center gap-2 text-sm text-amber-600 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 transition-colors py-2 px-4"
              >
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ t('dashboard.description') }}</p>
        </div>
      ...
      <section>
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
          <!-- Default Toolchain -->
          <div
            class="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 hover:shadow-sm transition-shadow"
          >
            <div class="flex items-center gap-3 mb-3">
              ...
              <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">{{
                t('dashboard.card.defaultToolchain')
              }}</span>
            </div>
            <p class="text-lg font-semibold text-gray-900 dark:text-gray-100 truncate" :title="defaultToolchain">
              {{ defaultToolchain }}
            </p>
          </div>
          ...
          >
            <div class="flex items-center gap-3 mb-3">
              <div
                class="w-9 h-9 rounded-lg bg-violet-50 dark:bg-violet-900/30 flex items-center justify-center shrink-0"
              >
                <iconify-icon icon="mdi:layers-outline" width="18" class="text-violet-600 dark:text-violet-400"></iconify-icon>
              </div>
              <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">{{
                t('dashboard.card.installed')
              }}</span>
            </div>
          ...
          >
            <template v-if="updateError">
              <div class="flex items-center gap-3 mb-3">
                <div
                  class="w-9 h-9 rounded-lg bg-amber-50 dark:bg-amber-900/30 flex items-center justify-center shrink-0"
                >
                  <iconify-icon icon="mdi:wifi-off" width="18" class="text-amber-600 dark:text-amber-400"></iconify-icon>
                </div>
                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">{{
                  t('dashboard.card.updates')
                }}</span>
              </div>
              <p class="text-sm text-amber-600 dark:text-amber-400 leading-snug">
                {{ t('dashboard.status.networkError') }}
              </p>
            </template>
            <template v-else>
              <div class="flex items-center gap-3 mb-3">
                <div
                  ...
                  :class="updateCount > 0 ? 'bg-yellow-50 dark:bg-yellow-900/30' : 'bg-green-50 dark:bg-green-900/30'"
                >
                  <iconify-icon
                    :icon="updateCount > 0 ? 'mdi:bell-outline' : 'mdi:check-circle-outline'"
                    width="18"
                    :class="
                      updateCount > 0 ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400'
                    "
                  ></iconify-icon>
                </div>
                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">{{
              ...
              <p
                ...
                :class="updateCount > 0 ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400'"
              >
                {{
                  updateCount > 0
                    ? t('dashboard.status.updatesAvailable', { count: updateCount })
                    : t('dashboard.status.upToDate')
                }}
              </p>
            </template>
          </div>
          ...
          >
            <div class="flex items-center gap-3 mb-3">
              <div
                class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                :class="
                  envCheck?.rustup_installed ? 'bg-green-50 dark:bg-green-900/30' : 'bg-amber-50 dark:bg-amber-900/30'
                "
              >
                <iconify-icon
                  :icon="envCheck?.rustup_installed ? 'mdi:shield-check-outline' : 'mdi:alert-outline'"
                  width="18"
                  :class="
                    envCheck?.rustup_installed
              ...
              <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">{{
                t('dashboard.card.environment')
              }}</span>
            </div>
            <p
              class="text-lg font-semibold truncate"
              :class="
                envCheck?.rustup_installed ? 'text-green-600 dark:text-green-400' : 'text-amber-600 dark:text-amber-400'
              "
            >
      ...
      <!-- Version info + Quick links (side by side on large screens) -->
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div class="space-y-3">
            ...
            >
              ...
              <span class="text-sm font-mono text-gray-900 dark:text-gray-100">{{
                versions.rustup_version || t('common.status.na')
              }}</span>
            </div>
            <div
              class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 flex items-center justify-between"
            >
              ...
              <span class="text-sm font-mono text-gray-900 dark:text-gray-100">{{
                versions.cargo_version || t('common.status.na')
              }}</span>
            </div>
          </div>
        </section>
        ...
        <section class="lg:col-span-2 space-y-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <router-link
              to="/toolchains"
              class="group bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 hover:border-sky-400 dark:hover:border-sky-600 hover:shadow-sm transition-all block"
            >
              <div
                class="w-10 h-10 rounded-lg bg-sky-50 dark:bg-sky-900/30 flex items-center justify-center mb-3 group-hover:bg-sky-100 dark:group-hover:bg-sky-900/50 transition-colors"
              >
            ...
            >
              <div
                class="w-10 h-10 rounded-lg bg-green-50 dark:bg-green-900/30 flex items-center justify-center mb-3 group-hover:bg-green-100 dark:group-hover:bg-green-900/50 transition-colors"
              >
            ...
            >
              <div
                class="w-10 h-10 rounded-lg bg-violet-50 dark:bg-violet-900/30 flex items-center justify-center mb-3 group-hover:bg-violet-100 dark:group-hover:bg-violet-900/50 transition-colors"
              >
              ...
              <h3 class="font-semibold text-gray-900 dark:text-gray-100 text-sm">
                {{ t('dashboard.label.componentsTargets') }}
              </h3>
              <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">
                {{ t('dashboard.action.addRemoveComponents') }}
              </p>
            </router-link>
            <!-- Uninstall rustup -->
            <button
              v-if="envCheck?.rustup_installed"
              class="group bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 hover:border-red-400 dark:hover:border-red-600 hover:shadow-sm transition-all block text-left cursor-pointer"
              @click="triggerUninstall"
            >
              <div
                class="w-10 h-10 rounded-lg bg-red-50 dark:bg-red-900/30 flex items-center justify-center mb-3 group-hover:bg-red-100 dark:group-hover:bg-red-900/50 transition-colors"
              >
